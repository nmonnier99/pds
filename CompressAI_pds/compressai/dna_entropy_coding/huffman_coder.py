...
from compressai.dna_entropy_coding.abstract_coder import AbstractCoder
...
class HuffmanCoder(AbstractCoder):
    ...
    def encode(self, inp):
        ...
        zipped = [(self.dic, e) for e in inp]
        encoded = pool.map(encode_element, zipped)
    ...
    def decode(self, code):
        ...
        while code != "":
            for i in range(0, min(len(code)+1, max_len_codeword)):
